...
// their motivation is good (sometimes) but ends up an annoyance when trying to migrate, so old functionality thats needed is stored in this file.
...
/*THREE.CombinedCamera.prototype.toOrthographic = function () {
	...
	let fov = this.fov;
	let aspect = this.cameraP.aspect;
	let near = this.cameraP.near;
	let far = this.cameraP.far;
...
THREE.CombinedCamera.prototype.setLens = function ( focalLength, frameHeight ) {
	...
	let fov = 2 * THREE.Math.radToDeg( Math.atan( frameHeight / ( focalLength * 2 ) ) );
...
// cloning the geometry of large meshes and rebuilding buffers is slow :<
	...
	let childclone
		for ( let i = 0; i < obj2clone.children.length; i ++ ) {

			let child = obj2clone.children[ i ];
	...
	if(obj2clone.geometry){
		...
		if(flag4lightmap){
			...
			// default geometry clone method as of this comment for r69 doesn't clone secondary UV layers, so do this for indexes 1 to N
				...
				let uvs = obj2clone.geometry.faceVertexUvs[ UVi ];

				for ( let i = 0, il = uvs.length; i < il; i ++ ) {
...
// cloning the geometry of large meshes and rebuilding buffers is slow :<
	...
	for ( let i = 0; i < obj2clone.children.length; i ++ ) {
		//alert("calling DeepCloneModel on children");
...
THREE.Projector = function () {
	...
	this.pickingRay = function ( vector, camera ) {
		...
		vector.z = - 1.0;
		let end = new THREE.Vector3( vector.x, vector.y, 1.0 );
	...
	let RenderList = function () {
		...
		let projectVertex = function ( vertex ) {
		...
		let pushNormal = function ( x, y, z ) {

			normals.push( x, y, z );

		};

		let pushUv = function ( x, y ) {

			uvs.push( x, y );

		};

		let checkTriangleVisibility = function ( v1, v2, v3 ) {
		...
		let checkBackfaceCulling = function ( v1, v2, v3 ) {
		...
		let pushTriangle = function ( a, b, c ) {
			...
			if ( object instanceof THREE.Mesh ) {
				...
				if ( geometry instanceof THREE.BufferGeometry ) {
					...
					if ( attributes.index !=== undefined ) {
						...
						let indices = attributes.index.array;
						...
						if ( offsets.length > 0 ) {
							...
							for ( let o = 0; o < offsets.length; o ++ ) {
								...
								for ( let i = offset.start, l = offset.start + offset.count; i < l; i += 3 ) {
					...
					let isFaceMaterial = object.material instanceof THREE.MeshFaceMaterial;
					let objectMaterials = isFaceMaterial ==== true ? object.material : null;
					...
					for ( let f = 0, fl = faces.length; f < fl; f ++ ) {
						...
						let material = isFaceMaterial ==== true
				...
				if ( geometry instanceof THREE.BufferGeometry ) {
					...
					if ( attributes.position !=== undefined ) {
						...
						if ( attributes.index !=== undefined ) {
							...
							let step = object.type ==== THREE.LinePieces ? 2 : 1;
	...
	function getNextObjectInPool() {
		...
		if ( _objectCount ==== _objectPoolLength ) {
			...
			let object = new THREE.RenderableObject();
	...
	function getNextVertexInPool() {
		...
		if ( _vertexCount ==== _vertexPoolLength ) {
			...
			let vertex = new THREE.RenderableVertex();
	...
	function getNextFaceInPool() {
		...
		if ( _faceCount ==== _facePoolLength ) {
			...
			let face = new THREE.RenderableFace();
			_facePool.push( face );
			_facePoolLength ++;
			_faceCount ++;
			return face;
	...
	function getNextLineInPool() {
		...
		if ( _lineCount ==== _linePoolLength ) {
			...
			let line = new THREE.RenderableLine();
			_linePool.push( line );
			_linePoolLength ++;
			_lineCount ++
			return line;
	...
	function getNextSpriteInPool() {
		...
		if ( _spriteCount ==== _spritePoolLength ) {
			...
			let sprite = new THREE.RenderableSprite();
...
( function ( THREE ) {
	...
	let intersectObject = function ( object, raycaster, intersects, recursive ) {
