		...
		if ( xhr.readyState ==== xhr.DONE ) {
			...
			if ( xhr.status ==== 200 || xhr.status ==== 0 ) {
				...
				if ( xhr.responseText ) {
					...
					let vertexShader = xhr.responseText;
					//let rexp = /<vertexShader>([\s\S]+?)<\/vertexShader>/
					...
					//let result = context.parse( json, texturePath );
					let result = context.objify(xhr.responseText);
