...
// not sure if using these is a good idea, or if it'll make stuff slower than necessary;
   ...
   /* let old_function = instanceProto.set_bbox_changed;
    ...
    instanceProto.set_bbox_changed = function() 
	{
        ...
        //let result = old_adder.apply(this, arguments);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	instanceProto.updateTNB = function ()   
	{
		if(!this.TNB.needsUpdate) return;
			...
			let inv, e = this.obj.matrixWorld.elements;
	...
	instanceProto.setupObj = function (SFLAG)   // call at some point in onCreate , preferrably near the beginning to ensure stuff for actions/conditions exists;
	{
		if(this.recycled){
	 ...
	 let old_function = this.set_bbox_changed;
	...
	{		
		for(let i = this.obj.children.length-1 ; i > -1 ; i--){
			if(this.obj.children[i].userData.inst){
				let sol = this.obj.children[i].userData.inst.type.getCurrentSol();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
		let sol,i;
		let this_sol = this.type.getCurrentSol(); // can't overwrite original selected object list for this type
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	Acts.prototype.ObjSetPosLocal = function (xpos,ypos,zpos)
	{
		...
		this.fastWorldUpdate();
		let e = this.obj.matrix.elements
	...
	Acts.prototype.ObjLookAtWorld = function (xpos,ypos,zpos)
	{
		...
		if(this.obj.parent.userData.inst){
			...
			let e = this.obj.parent.matrixWorld.elements
	...
	{
		let p = parent.getFirstPicked()
		if (!p) return;
		if(transf ==== 0){ //if yes
		this.fastWorldUpdate();
		p.fastWorldUpdate();
		let m = THREE.TSH.mtx0;
	...
	{
		let inst = childtype.getCurrentSol().getObjects();
		let i, leni;

		if(transf ==== 0){
			this.fastWorldUpdate();
			let m0 = THREE.TSH.mtx0;
			let m1 = THREE.TSH.mtx1;
